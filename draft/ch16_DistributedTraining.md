...
## 16.0 워밍업 — 여러 팀이 한 프로젝트를 나눠 맡는 일

본격적인 분산 학습 이야기에 들어가기 전에, 익숙한 비유 하나를 깔아 두겠다. 회사에서 1년짜리 대형 프로젝트를 한 사람에게 맡기면 그 사람은 죽는다. 그래서 보통 네 팀으로 쪼갠다. 어떤 팀은 같은 기능을 여러 고객용으로 **복제**해 병렬로 돌리고(Data Parallel), 어떤 팀은 거대한 문서를 여러 사람이 **페이지를 나눠** 쓰고(FSDP), 어떤 팀은 한 문장 안의 단어를 **여러 사람이 나눠 채우며**(Tensor Parallel), 또 다른 팀은 문서의 챕터 1·2·3 을 **순차 파이프라인**으로 돌린다(Pipeline Parallel). 네 팀이 따로 잘하는 일이 다르고, 실제 큰 프로젝트는 이 네 가지를 **조합** 한다. 분산 학습도 정확히 같은 구조를 가진다. 이 장에서 나올 DDP · FSDP · TP · PP 는 모두 이 네 팀의 이름표이고, GPT-4 · FSD 같은 초거대 모델은 이 팀들을 3차원 · 4차원으로 엮어 돌리는 거대 조직이다.
